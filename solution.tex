...
\begin{enumerate}
    \item Step 1: Convert the dataset from months into corresponding T years. Construct the the bias-corrected slope estimate $\hat{\phi}^{c} = \hat{\phi} + (1/T) \cdot (1+ 3 \cdot \hat{\phi}) + (1/T^2) \cdot 3 \cdot (1+ 3 \cdot \hat{\phi})$. 
